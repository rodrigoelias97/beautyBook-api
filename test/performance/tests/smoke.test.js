...
export default function () {
  ...
  check(healthResponse, {
    ...
    'health retorna status ok': (response) => response.json('status') === 'ok'
  });

  const token = obterToken();

  check(token, {
    'login retorna accessToken': (value) => Boolean(value)
  });
  ...
  check(serviceResponse, {
    ...
    'servicos retorna lista': (response) => Array.isArray(response.json())
  });
  ...
  check(availabilityResponse, {
    ...
    'availability retorna horarios': (response) => Array.isArray(response.json('horariosDisponiveis'))
  });
  ...
  check(appointmentResponse, {
    ...
    'appointment confirmado': (response) => response.json('status') === 'CONFIRMED'
  });
